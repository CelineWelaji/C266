...
def upload_image():
    ...
    if operation_selection == 'gray':
        ...
        file_data = imageSketch(decode_array_to_img)
    else:
...
def imageSketch(decode_array_to_img):
    converted_gray_img = cv2.cvtColor(decode_array_to_img,cv2.COLOR_RGB2GRAY)
    sharpingGrayImage = cv2.bitwise_not(converted_gray_img)
    blurImg = cv2.GaussianBlur(sharpingGrayImage,(111,111),0)
    sharpingBlurImage = cv2.bitwise_not(blurImg)
    sketchImg = cv2.divide(converted_gray_img,sharpingBlurImage,scale=256.0)
    status, output_image=cv2.imencode('.PNG',sketchImg)
